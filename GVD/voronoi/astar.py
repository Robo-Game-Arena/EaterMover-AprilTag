...
from GVD.voronoi.voronoi import Result
from GVD.voronoi.dictionary import IndexDict
from GVD.voronoi.geometry import *
...
class Astar:
    ...
    def __init__(self, vor: Result, start, end, fake) -> None:
        self.__vor = vor
        self.__dict = IndexDict(vor.ridge_vertices)
        self.__start = self.__add_ridge(start)
        self.__end = self.__add_ridge(end)
        self.__fake = self.__add_ridge(fake) # easy way to add fake goal into graph
